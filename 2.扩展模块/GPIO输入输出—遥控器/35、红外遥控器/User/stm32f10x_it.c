...
// IO 线中断，中断口为PD12 接红外接收头的数据管脚
void macIrDa_EXTI_INT_FUNCTION(void)
{
	uint8_t pulse_time = 0;
  uint8_t leader_code_flag = 0; /* 引导码标志位，当引导码出现时，表示一帧数据开始 */
  uint8_t irda_data = 0;        /* 数据暂存位 */
  
  if ( EXTI_GetITStatus ( macIrDa_EXTI_LINE ) != RESET ) /* 确保是否产生了EXTI Line中断 */
	{   
    while(1)
    {
      if( macIrDa_DATA_IN() == SET )        /* 只测量高电平的时间 */
      {       
        pulse_time = IrDa_Get_Pulse_Time();
		...
		EXTI_ClearITPendingBit ( macIrDa_EXTI_LINE );     //清除中断标志位
    //LED2_TOGGLE;
	}  
}
