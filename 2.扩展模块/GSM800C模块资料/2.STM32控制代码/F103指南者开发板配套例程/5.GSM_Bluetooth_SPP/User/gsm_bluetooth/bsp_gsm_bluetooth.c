    ...
    if(gsm_bt_mode==SPP)
    {
        ...
        if(strstr(redata,"+BTSPPDATA:")!=NULL)
        {
            if(strstr(redata,"\r\n")!=NULL)
            {
                ...
                printf("\r\n设备序号%s",strtok(redata,","));
                printf("\r\n数据长度:%s",strtok(NULL,","));
                printf("\r\n数据内容:%s",strtok(NULL,"\r"));
                GSM_CLEAN_RX();
            }
        }
        timecount++;
        if(timecount>=0x50000)//周期发送数据
        {   
            timecount = 0;
            gsm_cmd("AT+BTSPPSEND\r",">",100);//通过蓝牙串口发送数据
            sprintf((char *)cmdbuff,"BT SPP Mode test %d \r\n",sendcount);
            sendcount++;
            gsm_cmd((char *)cmdbuff,0,0);//发送数据
            gsm_cmd(&end,0,100); //发送结束符
            GSM_CLEAN_RX();
        }
    }
    else if(gsm_bt_mode==SPP)
    {
        printf("连接模式为蓝牙耳机，不能进行蓝牙串口通信，请重复操作");
    }
