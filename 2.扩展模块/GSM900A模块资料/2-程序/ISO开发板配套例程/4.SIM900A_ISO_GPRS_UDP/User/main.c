...
const char 	*TXBUF="秉火GSM模块UDP数据上传功能测试";
...
int main(void)
{
	static uint8_t timecount=0,timestop=0;
    /* 配置USART1 */
    USART1_Config();
	...
	if(sim900a_gprs_udp_link(LOCALPORT,SERVERIP,SERVERPORT)!=SIM900A_TRUE)
	{
		printf("\r\nUDP链接失败，请检测正确设置各个模块\r\n");
		SIM900A_DELAY(5000); 
		printf("\nIP链接断开\n");
	...
	printf("\r\nUDP链接成功，尝试发送数据\n");	
	if(sim900a_gprs_send(TXBUF)!=SIM900A_TRUE)
	{
