...
uint8_t linkHC05(void)
{
	...
	{
		if(strstr(bltDevList.name[i],"Bluetooth_Keyboard") != NULL) //非NULL表示找到有名称部分为HC05的设备
		{
			HC05_INFO("搜索到远程模块，即将进行配对连接...");
			...
			delay_ms(10);
		Usart_SendString(HC05_USART,"AT+PAIR=Bluetooth_Keyboard,5");
			...
			{ 
				...
				if(len>0)
				{
					if(redata[0]!=0)
					{
						HC05_DEBUG("send CMD: %s","AT+PAIR=Bluetooth_Keyboard,5");
