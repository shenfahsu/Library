...
BaseType_t xTaskGenericCreate( TaskFunction_t pxTaskCode,              //任务函数指针
                               const char * const pcName,              //命名任务
															 const uint16_t usStackDepth,            //任务栈深度
															 void * const pvParameters,              //传递给任务函数的实参
															 UBaseType_t uxPriority,                 //任务优先级
															 TaskHandle_t * const pxCreatedTask,     //任务句柄
															 StackType_t * const puxStackBuffer,     //NULL
															 const MemoryRegion_t * const xRegions ) //NULL
{
BaseType_t xReturn;
TCB_t * pxNewTCB;
StackType_t *pxTopOfStack;
	...
	if( pxNewTCB != NULL )                                      //如果分配成功
	{
		#if( portUSING_MPU_WRAPPERS == 1 )                        //如果定义了处理器封装，特权模式
			/* Should the task be created in privileged mode? */
			BaseType_t xRunPrivileged;                              
			if( ( uxPriority & portPRIVILEGE_BIT ) != 0U )          //如果任务优先级不为0
			{
				xRunPrivileged = pdTRUE;                              //设置优先权
			}
			else                                                    //如果任务优先级为0
			{
				xRunPrivileged = pdFALSE;                             //设置任务不具有优先权
			}
		...
		#if( portSTACK_GROWTH < 0 )          //如果堆栈从高到低增长   
		{
			pxTopOfStack = pxNewTCB->pxStack + ( usStackDepth - ( uint16_t ) 1 ); //栈顶位于最后一个堆栈单元
			pxTopOfStack = ( StackType_t * ) ( ( ( portPOINTER_SIZE_TYPE ) pxTopOfStack ) & ( ~( ( portPOINTER_SIZE_TYPE ) portBYTE_ALIGNMENT_MASK ) ) ); /*lint !e923 MISRA exception.  Avoiding casts between pointers and integers is not practical.  Size differences accounted for using portPOINTER_SIZE_TYPE type. */

			/* Check the alignment of the calculated top of stack is correct. */
			configASSERT( ( ( ( portPOINTER_SIZE_TYPE ) pxTopOfStack & ( portPOINTER_SIZE_TYPE ) portBYTE_ALIGNMENT_MASK ) == 0UL ) );
		}
		#else /* portSTACK_GROWTH */            //如果堆栈从低到高增长
		{
			pxTopOfStack = pxNewTCB->pxStack;       //栈顶位于首个堆栈单元
			...
			pxNewTCB->pxEndOfStack = pxNewTCB->pxStack + ( usStackDepth - 1 );//栈底位于最后一个堆栈单元
		}
