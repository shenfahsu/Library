...
static void vTaskWrite( void * pvParameters )             //写任务
{
	while(1)
	{
		xSemaphoreTakeRecursive( xSemaphore, portMAX_DELAY );  //获取递归互斥信号量 xSemaphore
		xSemaphoreTakeRecursive( xSemaphore, portMAX_DELAY );  //嵌套递归互斥信号量 xSemaphore
		...
		xSemaphoreGiveRecursive( xSemaphore );                 //给出递归互斥信号量 xSemaphore
		xSemaphoreGiveRecursive( xSemaphore );                 //相应解嵌2层
