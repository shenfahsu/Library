...
#ifdef MII_MODE /* Mode MII with STM322xx-EVAL  */
 #ifdef PHY_CLOCK_MCO
...
#elif defined RMII_MODE  /* Mode RMII with STM322xx-EVAL */
...
void Eth_Link_IT_task( void * pvParameters )
{
  ...
  xSemaphoreTake( ETH_link_xSemaphore, 0);
  for(;;)
  {
    if (xSemaphoreTake( ETH_link_xSemaphore, emacBLOCK_TIME_WAITING_ETH_LINK_IT)==pdTRUE)
    {
      /* Check whether the link interrupt has occurred or not */
      if(((ETH_ReadPHYRegister(pcPHYAddress, PHY_MISR)) & PHY_LINK_STATUS) != 0)
      {
        if((ETH_ReadPHYRegister(pcPHYAddress, PHY_SR) & 1))
        {
