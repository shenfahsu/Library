...
"%FFMPEG%" -y -i %1 -r %FRAMERATE% -q %QUALITY% -s %SIZE% -f image2 -pix_fmt yuvj420p "%FOLDER%img-%%05d.jpeg"

"%JPEG2MOVIE%" "%FOLDER%"
IF ERRORLEVEL 1 GOTO ERROR
GOTO NOERROR
:ERROR
ECHO Error on using JPEG2Movie!
PAUSE
:NOERROR

COPY /B %FOLDER%*.emf %~dp1%~n1_%SIZE%.emf
