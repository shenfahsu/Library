...
void  OSTaskStkChk (OS_TCB        *p_tcb,                  //目标任务控制块的指针
                    CPU_STK_SIZE  *p_free,                 //返回空闲堆栈大小
                    CPU_STK_SIZE  *p_used,                 //返回已用堆栈大小
                    OS_ERR        *p_err)                  //返回错误类型
{
...
#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u             //如果使能了中断中非法调用检测
    if (OSIntNestingCtr > (OS_NESTING_CTR)0) {     //如果该函数是在中断中被调用
       *p_err = OS_ERR_TASK_STK_CHK_ISR;           //错误类型为“在中断中检测堆栈”
        return;                                    //返回，停止执行
    }
#endif
    ...
    if (p_tcb->StkPtr == (CPU_STK*)0) {                     //如果目标任务的堆栈为空
        CPU_CRITICAL_EXIT();                                //开中断
    ...
    /* 如果目标任务的堆栈非空 */
    if ((p_tcb->Opt & OS_OPT_TASK_STK_CHK) == (OS_OPT)0) {  //如果目标任务没选择检测堆栈
        CPU_CRITICAL_EXIT();                                //开中断
       *p_free = (CPU_STK_SIZE)0;                           //清零 p_free
       *p_used = (CPU_STK_SIZE)0;                           //清零 p_used
       *p_err  =  OS_ERR_TASK_OPT;                          //错误类型为“任务选项有误”
        return;                                             //返回，停止执行
    }
    CPU_CRITICAL_EXIT();                                    //如果任务选择了检测堆栈，开中断
    /* 开始计算目标任务的堆栈的空闲数目和已用数目 */
    free_stk  = 0u;                                         //初始化计算堆栈工作
#if CPU_CFG_STK_GROWTH == CPU_STK_GROWTH_HI_TO_LO           //如果CPU的堆栈是从高向低增长
    p_stk = p_tcb->StkBasePtr;                              //从目标任务堆栈最低地址开始计算
    while (*p_stk == (CPU_STK)0) {                          //计算值为0的堆栈数目
        p_stk++;
        free_stk++;
    }
#else                                                       //如果CPU的堆栈是从低向高增长
    p_stk = p_tcb->StkBasePtr + p_tcb->StkSize - 1u;        //从目标任务堆栈最高地址开始计算
    while (*p_stk == (CPU_STK)0) {                          //计算值为0的堆栈数目
        free_stk++;
        p_stk--;
    }
#endif
   *p_free = free_stk;                                      //返回目标任务堆栈的空闲数目
   *p_used = (p_tcb->StkSize - free_stk);                   //返回目标任务堆栈的已用数目
   *p_err  = OS_ERR_NONE;                                   //错误类型为“无错误”
}
#endif
